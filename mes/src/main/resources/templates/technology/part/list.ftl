            ...
            <div class="layui-form-item">
                <div class="layui-inline">
                    ...
                    <div class="layui-input-inline">
                        <input type="text" name="partNoLike" autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-inline">
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable', 'spLayui'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/technology/part/page',
            cols: [
                [{
                type: 'checkbox'
            }, {
                    field: 'partNo', title: '零部件编号'
                }, {
                    field: 'partName', title: '零部件名称'
                }, {
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'deleteBatch') {
                ...
                if (data.length > 0) {
                    layer.confirm('确认要删除吗？', function (index) {
                        ...
                        spUtil.ajax({
                            url: '${request.contextPath}/technology/part/delete-batch',
                            type: 'POST',
                            data: {ids: ids.join(',')},
                            success: function () {
                                layer.close(index);
            ...
            if (obj.event === 'add') {
                var index = spLayer.open({
                    title: '添加零部件',
                    area: ['70%', '90%'],
                    content: '${request.contextPath}/technology/part/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/technology/part/delete',
                        async: false,
                        type: 'POST',
                        showLoading: true,
                        serializable: false,
